...
(function() {
    ...
    window.speakText = function(text, event) {
        ...
        if ('speechSynthesis' in window) {
            ...
            // Android/Chrome Bugfix: Kurze Pause nach cancel() + globale Variable, damit es nicht vom Garbage Collector gefressen wird
            setTimeout(() => {
                window.currentUtterance = new SpeechSynthesisUtterance(cleanText);
                window.currentUtterance.lang = 'de-DE';
                window.currentUtterance.rate = 0.9; // Etwas langsamer für Kinder
                window.currentUtterance.pitch = 1.2; // Etwas hellere, freundlichere Stimme
